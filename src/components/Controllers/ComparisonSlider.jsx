...
const ComparisonSlider = ({ isVisible, sidebarExpanded, position, comparisonStartDate, comparisonEndDate, setPosition }) => {
    ...
    if (!isVisible)
        return null;
    return (<motion.div className="absolute top-0 h-full z-[1000] select-none" animate={{ left: `${position * 100}%` }} transition={{ duration: 0.1 }}>
      {/* Start Date */}
      <div className="absolute top-[30px] right-[50px] flex justify-end w-[300px]">
        <div className="bg-bg-quantinary/30 shadow-xl border-[1px] border-border-secondary/40 backdrop-blur-sm backdrop-filter h-fit flex justify-end px-3 py-1 rounded-full">
          <Text textColor={theme.colors.bg.primary} size={theme.text.size.body2} bold={theme.text.bold.md2}>
      ...
      {/* Drag handle */}
      <div onMouseDown={handleMouseDown} className="absolute top-[50%] left-[-20px] w-[40px] h-[40px] rounded-full bg-white border border-gray-300 shadow-md flex items-center justify-center cursor-col-resize -translate-y-1/2">
