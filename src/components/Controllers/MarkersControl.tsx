...
const DateRanges = ["Today", "Yesterday", "Last 7 days", "Last month"];
    ...
    >
      ...
      >
        <div className="w-full h-[21px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-gray-500">
            ...
            {isCollapsed ? (
              <FaChevronRight size={15.06} className="cursor-pointer" onClick={toggleCollapse} />
            ) : (
              <FaChevronDown size={16.06} className="cursor-pointer" onClick={toggleCollapse} />
            )}
          </div>
        </div>
        ...
        {!isCollapsed && (
          <div className="mt-1 flex flex-col gap-1">
            <div className="w-full h-[30px] bg-white border border-[var(--color-border-primary)] rounded-[8px] px-2 flex items-center gap-2">
              ...
              <input type="text" placeholder="Search district..." className="w-full text-sm text-gray-700 placeholder-gray-400 outline-none" />
            </div>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 h-[20px] px-2.5 rounded-full border text-xs font-medium transition ${
                      isSelected ? "border-[var(--color-main-primary)] text-[var(--color-main-primary)]" : "border-gray-300 text-gray-500"
                    }`}
                  >
      ...
      >
        <div className="w-full h-[21px] flex items-center justify-between">
          ...
          {isMiningCollapsed ? (
            <FaChevronRight size={15.06} className="cursor-pointer text-gray-500" onClick={() => setIsMiningCollapsed(false)} />
          ) : (
            <FaChevronDown size={16.06} className="cursor-pointer text-gray-500" onClick={() => setIsMiningCollapsed(true)} />
          )}
        </div>
        ...
        {!isMiningCollapsed && (
          <div className="mt-2">
            <div className="w-full h-[30px] bg-white border border-[var(--color-border-primary)] rounded-[8px] px-2 flex items-center gap-2">
              ...
              <input type="text" placeholder="Search company..." className="w-full text-sm text-gray-700 placeholder-gray-400 outline-none" />
            </div>
          </div>
        )}
      </motion.div>
      ...
      <motion.div
        animate={{ height: isDateCollapsed ? 35 : 241 }}
        transition={{ duration: 0.3, ease: "easeInOut" }}
        className="w-full overflow-hidden bg-white/50 backdrop-blur-xs shadow-md px-2 py-[5px] rounded-[10px] mt-2 "
      >
        <div className="w-full h-[21px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-gray-500">
            ...
            {isDateCollapsed ? (
              <FaChevronRight size={15.06} className="cursor-pointer" onClick={() => setIsDateCollapsed(false)} />
            ) : (
              <FaChevronDown size={16.06} className="cursor-pointer" onClick={() => setIsDateCollapsed(true)} />
            )}
          </div>
        </div>
          ...
          <div className="mt-3 flex flex-col gap-3">
            <div className="flex justify-between px-[2px]">
              ...
              <div className="flex flex-col w-[82px]">
                <Text size={theme.text.size.SM} bold={theme.text.bold.sm} className="!text-[var(--color-text-tetiary)]">From</Text>
                <div
              ...
              {/* TO */}
              <div className="flex flex-col w-[82px]">
                <Text size={theme.text.size.SM} bold={theme.text.bold.sm} className="!text-[var(--color-text-tetiary)]">To</Text>
                <div
                  className="flex items-center w-[81px] h-[28px] border-[var(--border-tetiary)] rounded-[10px] justify-around px-[4px] border-[1px] bg-white/50 cursor-pointer"
            ...
            <div className="flex flex-col gap-1.5 px-1">
              {DateRanges.map((label) => (
                <button
                  ...
                  className={`w-full h-[28px] rounded-[7px] text-[13px] font-medium transition-all
                    ${activeRange === label
