...
import { UserTable } from "@components/Accounts/user-table";
...
const UserTableSection = ({ filteredUsers, onStatusChange, onRoleChange, onDepartmentChange, onRemoveUser, showInviteModal, setShowInviteModal, onInviteUser, }) => {
    ...
    const { register: registerEdit, handleSubmit: handleSubmitEdit, formState: { errors: errorsEdit }, reset: resetEdit, setValue: setEditValue } = useForm({
    ...
    const columns = [
        {
            ...
            header: "Full Name",
            cell: ({ row }) => (<Text textColor="rgb(31 41 55)" size={TypographySize.body} bold={TypographyBold.sm2}>
        ...
        },
        {
            accessorKey: "email",
            header: "Email",
            cell: ({ row }) => (<Text textColor="rgb(31 41 55)" size={TypographySize.body} className="lowercase">
          {row.getValue("email")}
        </Text>),
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (<Badge variant={variant} className={`capitalize rounded-lg opacity-80 ${colorClass}`}>
            <Text textColor="inherit" size={TypographySize.body} bold={TypographyBold.sm2}>
              {role.toLowerCase()}
            </Text>
          </Badge>);
            },
        ...
        },
        {
            ...
            header: "Department",
            cell: ({ row }) => (<Text textColor="rgb(31 41 55)" size={TypographySize.body} className="capitalize">
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                if (!createdAt)
                    return (<Text textColor="rgb(156 163 175)" size={TypographySize.body}>
            unknown
          </Text>);
        ...
        },
        {
            ...
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleDeleteClick(row.original)} title="Delete user">
            <IconTrash className="h-4 w-4 text-red-500"/>
          </Button>
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleEditClick(row.original)} title="Edit user">
            <IconEdit className="h-4 w-4 text-gray-600"/>
            </Button>
            <div title={isActive ? "Set inactive" : "Set active"}>
              <Switch checked={isActive} onCheckedChange={handleToggleClick} className={`relative border cursor-pointer border-gray-200
                  data-[state=checked]:bg-green-300
                  data-[state=unchecked]:bg-red-300
                `}>
                <span className="absolute left-1 top-1/2 -translate-y-1/2 pointer-events-none transition-opacity duration-200" style={{ opacity: isActive ? 1 : 0 }}>
                  <IconCheck className="h-3 w-3 text-white"/>
                </span>
                <span className="absolute right-1 top-1/2 -translate-y-1/2 pointer-events-none transition-opacity duration-200" style={{ opacity: isActive ? 0 : 1 }}>
                  <IconX className="h-3 w-3 text-white"/>
                </span>
              </Switch>
            </div>
            {/* Confirmation Dialog */}
            {showConfirm && (<div className="fixed inset-0 z-50 flex items-center backdrop-blur-sm justify-center bg-opacity-30">
                <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button onClick={handleConfirm} className={pendingStatus === 'ACTIVE' ? "bg-[var(--color-main-primary)] text-white hover:bg-[var(--color-main-primary)]/90" : "bg-red-500 text-white hover:bg-red-600"}>
                      Confirm
                    </Button>
                  </div>
                </div>
    ...
    return (<div className="flex-1 bg-white rounded-xl shadow-sm border custom-scrollbar border-gray-200 overflow-hidden">
      {/* <Tabs defaultValue="overview" className="h-full flex flex-col"> */}
        {/* <TabsContent value="overview" className="flex-1 p-4 overflow-y-auto custom-scrollbar"> */}
          <UserTable data={filteredUsers} columns={columns} onStatusChange={onStatusChange} onRoleChange={onRoleChange} onDepartmentChange={onDepartmentChange} onRemoveUser={onRemoveUser} onInviteUser={onInviteUser}/>
 

      {/* Loading Spinner */}
      {isMutating && (<div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 flex items-center gap-2">
            <div className="animate-spin rounded-full h-6 w-6 border-2 border-[var(--color-main-primary)] border-t-transparent"/>
            <Text textColor="rgb(31 41 55)" size={TypographySize.body} bold={TypographyBold.sm2}>
              Updating...
            </Text>
          </div>
        </div>)}
      ...
      {/* Invite Modal */}
      {showInviteModal && (<div className="fixed inset-0 border border-gray-100 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md border border-gray-900">
            ...
            <form onSubmit={handleSubmit(handleSendInvite)}>
              <div className="mb-4">
                ...
                <CustomInput value={watchEmail} placeholder="Email address" {...register("email")} className="mb-1"/>
      ...
      {/* Edit User Modal */}
      {showEditModal && editingUser && (<div className="fixed inset-0 border border-gray-100 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md border border-gray-900">
            ...
            <form onSubmit={handleSubmitEdit(handleSaveEdit)}>
              <div className="mb-4">
                ...
                <Select value={editingUser.role} onValueChange={(value) => setEditValue('role', value)}>
              ...
              <div className="mb-4">
                ...
                <Select value={editingUser.department} onValueChange={(value) => setEditValue('department', value)}>
      ...
      {showDeleteModal && deletingUser && (<div className="fixed inset-0 border border-gray-100 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md border border-gray-900">
